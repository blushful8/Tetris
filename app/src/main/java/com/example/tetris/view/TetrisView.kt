...
class TetrisView : View {
    ...
    fun setGameCommand(move: AppModel.Motions) {
        if (null != model && (model?.currentState ==
            ...
            setGameCommandWidthDelay(move)
        }
    }

    fun setGameCommandWidthDelay(move: AppModel.Motions) {
    ...
    private fun updateScores() {
        ...
        activity?.tvHighScore?.text = "${
            activity?.appReferences?.getHighScore()
        }"
    }
            ...
            if (msg.what == 0) {
                if (owner.model != null) {
                    if (owner.model!!.isGameOver()) {
                        owner.model?.endGame()
                        Toast.makeText(owner.activity, "Game Over", Toast.LENGTH_SHORT)
                            .show()
                    }
                    if (owner.model!!.isGameActive()) {
                        owner.setGameCommandWidthDelay(AppModel.Motions.DOWN)
                    }
                }
            }
        }
